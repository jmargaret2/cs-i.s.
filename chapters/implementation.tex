...
%TODO: add limit of scope vs functions in the language
...
\begin{listing}
	\begin{lstlisting}
		...
		thirdFreq = thirdFreq + baseFrequency;
		fifthFreq = fifthFreq + baseFrequency;
	\end{lstlisting}
...
The distortion of a simple pure audio signal involves a SuperCollider class known as \textit{Inside Out}. For this module, both clipping and fuzz are used. As in Listing \ref{lst:distortion-waveforms}, there are two aspects which create the clipping and fuzz effects: a sine wave Unit Generator (UGen), and a \texttt{Pink Noise} UGen. The sine wave UGen allows for the clipping of the sound wave, at $1 - \textrm{input}$ (with input equal to \texttt{SinOsc.ar(baseFrequency)}), since we have a positive sine wave signal. The \texttt{Pink Noise} UGen is added through the fuzz effect, using the value 30 as the frequency multiplier. 

\begin{listing}
	\begin{lstlisting}
		// add distortion by using the PinkNoise function, with a new sine wave, with the same frequency (baseFrequency) from pitchKnob
...
\subsection{Adding Legato} %TODO: make Legato code consistent with what I have
...
\begin{listing}
	...
	\end{lstlisting}
	\caption{Creating a sustain button with MIDI compatibility in SuperCollider}
	\label{lst:legato-midi}	
\end{listing}

\begin{listing}
	\begin{lstlisting}
		while(legatoButton.value == 0,
			off = MIDIdef.noteOff(\myKeyUp, {arg vel, note;
			notesArray[note].set(\gate, 0);
			["NOTE OFF STATUS BUTTON", note].postln;
		});
	\end{lstlisting}
	\caption{The conditional to trigger MIDI note off after note on, or not}
	\label{lst:midi-trigger-legato}
\end{listing}
